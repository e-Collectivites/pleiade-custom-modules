  ...
  // The SOAP request XML
              ...
              foreach ($responseJson[0]['Body']['SearchResponse']['appt'] as &$appointment) {
                ...
                if (isset($appointment['recur']) && $appointment['recur'] == 1) {
                  ...
                  $ApptApiEndpoint = $domainToken->domain . 'service/soap';
                  $ApptRequestXml =
                    '<soap:Envelope xmlns:soap="http://www.w3.org/2003/05/soap-envelope">
                      <soap:Header>
                          <context xmlns="urn:zimbra">
                  ...
                  curl_setopt($ApptCurl, CURLOPT_URL, $ApptApiEndpoint);
                  curl_setopt($ApptCurl, CURLOPT_RETURNTRANSFER, true);
                  curl_setopt($ApptCurl, CURLOPT_POST, true);
                  curl_setopt($ApptCurl, CURLOPT_POSTFIELDS, $ApptRequestXml);
                  curl_setopt($ApptCurl, CURLOPT_HTTPHEADER, array('Content-Type: application/soap+xml'));
                  curl_setopt($ApptCurl, CURLOPT_SSL_VERIFYPEER, false); // Désactivez la vérification SSL (utilisez avec précaution)

                  // Exécutez la requête cURL

                  $ApptResponseXml = curl_exec($ApptCurl);
                  // Check for cURL errors
                  if (curl_errno($ApptCurl)) {
                    echo "cURL Error: " . curl_error($ApptCurl);
                  }

                  // Close cURL resource
                  curl_close($ApptCurl);
                  $responseApptJson = Json::decode($ApptResponseXml);
