          ...
          xhr.onload = function () {
            if (xhr.status === 200) {
              ...
              if (donnees) {
                var event_array = [];
                document.cookie =
                  "nbOfTasks=" + donnees.Body.SearchResponse.appt.length;
                for (
                  var i = 0;
                  i < donnees.Body.SearchResponse.appt.length;
                  i++
                ) {
                  var start_task =
                    donnees.Body.SearchResponse.appt[i].inst[0].s / 1000;
                  var end_task =
                    start_task + donnees.Body.SearchResponse.appt[i].dur / 1000;
                  ...
                  event_array[i] = {
                    ...
                    url:
                      ...
                      donnees.Body.SearchResponse.appt[i].invId +
                      "?utcRecurrenceId=" +
                      donnees.Body.SearchResponse.appt[i].inst[0].ridZ +
                      "&start=" +
                      donnees.Body.SearchResponse.appt[i].inst[0].s +
                      "&end=" +
                      end_task * 1000,
                    extendedProps: {
                      ...
                      longTitle:
                        "<b>" +
                        donnees.Body.SearchResponse.appt[i].name +
                        "</b>, <br><strong>Lieu : </strong>" +
                        donnees.Body.SearchResponse.appt[i].loc,
                    },
                  };
                }
                // debug
