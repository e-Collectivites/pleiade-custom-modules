...
use Drupal\api_humhub_pleiade\Service\HumhubServiceInterface;
...
class humHubController extends ControllerBase
{
        ...
        protected $service;
        public function __construct(HumhubServiceInterface $service)
        {
        ...
        {
                return new static(
                        $container->get(HumhubServiceInterface::class)
                );
        }
                ...
                if (mysqli_num_rows($result) > 0) {
                        ...
                        if ($ID_user !== NULL) {
                                ...
                                if ($token !== null) {
                                        ...
                                        if ($this->service->get_notif_humhub($token) != 'undefined') {
                                                $return['notifs'] = $this->service->get_notif_humhub($token);
                                        }
                                        if ($this->service->get_messages_humhub($token) != 'undefined') {
                                                $return['messages'] = $this->service->get_messages_humhub($token);
                                        }
                                        if ($this->service->get_spaces($token) != 'undefined') {
                                                $spaces = $this->service->get_spaces($token)["results"];
